...
exports.defineManualTests = function(contentEl, createActionButton) {
  ...
  var watchLocation = function(usePlugin) {
    ...
    // Get location
    watchLocationId = geo.watchPosition(success, fail, {
  ...
  var getLocation = function(usePlugin, opts) {
    ...
    // Get location
    geo.getCurrentPosition(success, fail, opts || {
        enableHighAccuracy: true
      }); //, {timeout: 10000});
  ...
  var location_div = '<div id="info">' +
    ...
    time = '<tr>' +
      ...
      '</table>' +
      '</div>',
    actions = '<h2>Use Built-in WebView navigator.geolocation</h2>' +
      ...
      'Expected result: Will update location values with a cached position that is up to 30 seconds old. Verify with time value. Status will read Done.',
    values_info = '<h3>Details about each value are listed below in the status box</h3>',
    note = '<h3>Allow use of current location, if prompted</h3>';
